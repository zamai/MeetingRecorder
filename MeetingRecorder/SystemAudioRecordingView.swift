        ...
        Task {
            do {
                ...
                guard let exportSession = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetAppleM4A) else {
                    completion(NSError(domain: "com.meetingrecorder.merger", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to create export session."]))
                    return
                }
...
extension URL {
    static var applicationSupport: URL {
        do {
            ...
            let subdir = appSupport.appending(path: "MeetingRecorder", directoryHint: .isDirectory)
